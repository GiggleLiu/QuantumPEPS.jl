...
function energy(qpeps, model::TFI; nbatch)
   energy(qpeps, Z, model; nbatch=nbatch) +
    energy(qpeps, X, model; nbatch=nbatch)
end

function energy(qpeps, pauli::ZGate, model::TFI; nbatch)
    res = gensample(qpeps, pauli; nbatch=nbatch)
    local eng = 0.0
...
function energy(qpeps, pauli::XGate, model::TFI; nbatch)
    res = gensample(qpeps, pauli; nbatch=nbatch)
